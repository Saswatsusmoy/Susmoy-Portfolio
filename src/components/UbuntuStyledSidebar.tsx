...
const SocialIcon = ({ href, children, label }: SocialIconProps) => (
  <a
    ...
    className="group flex items-center justify-center p-2 transition-colors duration-200 hover:bg-foreground/5 border-l-2 border-transparent hover:border-accent"
    aria-label={label}
  >
    <div className="w-5 h-5 flex items-center justify-center text-muted group-hover:text-foreground transition-colors duration-200">
      {children}
    </div>
  </a>
);
...
const EmailIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
    <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-.904.732-1.636 1.636-1.636h.91L12 10.364l9.454-6.543h.91c.904 0 1.636.732 1.636 1.636z"/>
  </svg>
);
  ...
  return (
    <div className="hidden md:block fixed left-2 bottom-4 z-40 lg:left-0">
      <div className={`
        ...
        ${theme === 'dark' ? 'border-border/30' : 'border-border/40'}
      `}>
        {/* Social links */}
        <div className="p-1">
