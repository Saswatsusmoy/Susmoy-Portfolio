...
const AICodingContent = () => (
  <div className="max-w-none text-[color:var(--foreground)] font-mono text-sm leading-relaxed">
    <div className="mb-8 p-4 bg-[color:var(--panel)] border-l-4 border-[color:var(--accent)] rounded-r-lg">
    ...
    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> The Before Times (A.K.A. The Tale of the Geek)
    </h2>
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
    ...
    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> 2020, Lockdown, HP Laptop, Charles Severance and Me
    </h2>
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
      It was 2020 when I got into programming. It started as a hobby back then but now it has become my bread n butter (sort of). I had a modest (2020 standard) HP laptop with a great internet and decided to start with python (because reddit told me so). Took up <a href="https://www.coursera.org/learn/python" target="_blank" rel="noopener noreferrer" className="text-[color:var(--accent)] hover:underline font-medium">Charles Severance&apos;s Coursera course (Python for Everybody)</a> and then started with <a href="https://youtu.be/LfaMVlDaQ24?si=ZQWn4SzJENn0zpTi" target="_blank" rel="noopener noreferrer" className="text-[color:var(--accent)] hover:underline font-medium">CS50</a> (I knew nothing about programming back then). Had a bit of fun with those Chuck lectures (mostly the slytherin jokes) and built a few basic things on Atom (but who cares right?).
    </p>

    <div className="mb-6 p-3 bg-[color:var(--panel)] border border-[color:var(--border)] rounded-lg">
      <p className="text-[color:var(--muted)] text-xs">
        <span className="text-[color:var(--accent)]">{"//"}</span> This was the time when I experienced (somewhat) the joy of rambling through those classic HTML aesthetic dev-forums, stack-overflow and what-not to get my idea into a working code. No coding assistants, no prompt engineering and no context engineering bullshit, just me and my minimal code.
      </p>
    </div>

    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> ChatGPT and I
    </h2>
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
    ...
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
      January 2023, A random physics workshop, assigned to take some readings and make inferences based on those readings. There sat my friend (<a href="https://salty-duck0.github.io/" target="_blank" rel="noopener noreferrer" className="text-[color:var(--accent)] hover:underline font-medium">check him out, he does some crazy RL stuff</a>) typing away on his laptop on a minimal green-black screen and the rest was history. It was ChatGPT and it was crazy. He typed his query and passed those readings and voila, the output streaming like a voice inside the laptop guiding him. Curious me, latched unto him and made him show me the ropes. It was a whole new world.
    </p>

    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> The project
    </h2>
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
    ...
    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> The Learning Decelerator
    </h2>
    <p className="text-[color:var(--muted)] mb-6 pl-4 border-l-2 border-[color:var(--border)]">
    ...
    <div className="bg-[color:var(--panel)] border border-[color:var(--border)] rounded-lg p-4 my-8">
      <h3 className="font-mono text-sm font-semibold mb-4 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
        <span className="text-[color:var(--accent)]">{"//"}</span> My New Workflow (The AI Version):
      </h3>
      <div className="space-y-2 text-sm text-[color:var(--muted)] font-mono">
    ...
    <h2 className="text-lg font-bold mt-12 mb-6 text-[color:var(--foreground)] border-b border-[color:var(--border)] pb-2">
      <span className="text-[color:var(--accent)]">{"//"}</span> The Honest Truth About AI Coding
    </h2>
    <div className="mb-6 p-4 bg-[color:var(--panel)] border-l-4 border-[color:var(--accent)] rounded-r-lg">
      <p className="text-[color:var(--foreground)] font-medium">
